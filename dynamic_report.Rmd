---
title: My dynamic report
output: html_fragment
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, error=FALSE)

```

```{css}
/* 自定义一些简单的页面样式 */
body {
  font-family: "Helvetica", sans-serif; 
  margin: 20px;
}
h1, h2, h3, h4 {
  color: #2c3e50;
  margin-top: 1.5em;
}
p {
  line-height: 1.5;
  margin: 0.8em 0;
}
img {
  max-width: 900px; /* 控制图片显示宽度 */
  height: auto;
  margin-bottom: 1em;
}

```

1. YouTube Watching Analysis In this report, we will import our own collection of YouTube video viewing log data.
and use R to draw histograms and calculate some summary statistics.
```{r}
# (A) 加载 tidyverse
library(tidyverse)

# (B) 导入数据
logged_data <- read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vTWuiHKezF7pWlcRSSSmoSOws0JEdIth5NqWsm1OurOrJQyKVBqBClSYf1pTJ-Z51zONqd6F2VJewlk/pub?output=csv")

# (C) 重命名列
latest_data <- logged_data %>%
  rename(
    time_stamp = TIME,
    main_type  = `What are the main types of videos you watch on YouTube?`,
    day_period = `What time of day do you usually choose to watch videos?`,
    minutes    = `How many minutes a day do you spend watching videos?`
  )

# (D) 计算汇总值
summary_values <- latest_data %>%
  summarise(
    min_minutes = min(minutes, na.rm = TRUE),
    max_minutes = max(minutes, na.rm = TRUE),
    avg_minutes = mean(minutes, na.rm = TRUE),
    total_obs   = n()
  )

# (E) 打印汇总表（仅用于检查，如果不想在成品里看表格，可注释掉）
summary_values

```
## 1.1 Summary of Watching Behavior

我们总共记录了 `r summary_values$total_obs` 次观看。
每次观看时长的最小值是 `r summary_values$min_minutes` 分钟，
最大值是 `r summary_values$max_minutes` 分钟；
平均下来大约 `r round(summary_values$avg_minutes, 1)` 分钟都花在了 YouTube 上。





```{r}
ggplot(latest_data, aes(x = main_type)) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Distribution of Main Video Types",
    x = "Video Type",
    y = "Count"
  ) +
  theme_minimal()
```
As can be seen, the bar chart above reflects the distribution of the types of videos watched.




```{r}
ggplot(latest_data, aes(x = day_period)) +
  geom_bar(fill = "tomato") +
  labs(
    title = "When Do We Usually Watch Videos?",
    x = "Time of Day",
    y = "Count"
  ) +
  theme_minimal()
```
This chart shows everyone's viewing habits at different times of the day.






```{r}
pets <- c("cat", "dog", "turtle", "rabbit")
best_pet <- pets[1]

```

The best pet to have is a `r best_pet` of course!

![](https://c.tenor.com/-qBsG1HwR4oAAAAC/tenor.gif)

![](https://c.tenor.com/-qBsG1HwR4oAAAAC/tenor.gif)
